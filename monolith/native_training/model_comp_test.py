...
import os
# MONOLITH_WITH_HOROVOD has to be set before monolith.native_training.
os.environ["MONOLITH_WITH_HOROVOD"] = "True"
#os.environ["MONOLITH_WITH_OPTIMIZED_HOROVOD"] = "True"
...
def lookup_tf_embedding(features, f_name, dim):
  f = tf.RaggedTensor.from_row_splits(features[f'tf_{f_name}_p1'], features[f'tf_{f_name}_p2'])
    ...
    def input_gen():
      while True:
        ...
        for i in range(num_features):
          features[f'feature{i}'] = np.random.randint(0, fid_max_val, size=random.randint(1, 25), dtype=np.int64)
        features['label'] = random.choice([0.0, 1.0])
        yield features

    def decomp_func(features):
      # note: this is a workaround to pass fids to model_fn, since all instances if RaggedTensor are gone
      for i in range(num_features):
        features[f'tf_feature{i}_p1'] = features[f'feature{i}'].values
        features[f'tf_feature{i}_p2'] = features[f'feature{i}'].row_splits
      return features
    
    output_signatures = {'label': tf.TensorSpec(())}
    for i in range(num_features):
      output_signatures[f'feature{i}'] = tf.TensorSpec((None,), dtype=tf.int64)
      
    return tf.data.Dataset.from_generator(input_gen, output_signature=output_signatures)\
      .apply(tf.data.experimental.dense_to_ragged_batch(batch_size, True))\
        .map(decomp_func).map(advanced_parse, tf.data.AUTOTUNE).prefetch(tf.data.AUTOTUNE)
        
    # def input_tensors():
    #   features = {}
    #   for i in range(num_features):
    #     features[f'feature{i}'] = tf.random.uniform((tf.random.uniform((), 1, 25, dtype=tf.int32),), 0, fid_max_val, dtype=np.int64)
    #   features['label'] = tf.cast(tf.random.uniform((), 0, 2, dtype=tf.int32), tf.float32)
    #   return features
    
    # return tf.data.Dataset.from_tensors(input_tensors()).repeat(102400000)\
    #   .apply(tf.data.experimental.dense_to_ragged_batch(batch_size, True))\
    #     .map(decomp_func).map(advanced_parse, tf.data.AUTOTUNE).prefetch(tf.data.AUTOTUNE)
